#!/bin/bash
# Simple bash script for delete airflow user.
...
create_airflow_user() {
  echo "Deleting Airflow user: $firstname $lastname"
  echo "Role: $rolename"
  echo "Username: $username"
  echo "Password: $password"
  echo "Email: $email"
  airflow users delete -f "$firstname" -l "$lastname" -r "$rolename" -u "$username" -p "$password" -e "$email"
}
